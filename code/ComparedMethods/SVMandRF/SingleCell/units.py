...
import tqdm
from code.ComparedMethods.SVMandRF.SingleCell import node2vec
...
def genenet_attribute_singleCell_all_feat(allx, all_dim="758"):
    if allx.shape[1] < all_dim:
        raise Exception("The dim of Gene Expression is less than feat_dim, please resetup the feat_dim!!!!!")
    if all_dim > 2:
        # Relative_variation = np.zeros((allx.shape[0], allx.shape[1] - 1))
        # for i in range(allx.shape[1] - 1):
        ...
        Relative_variation = scale(allx)
        #   提取cmd_args.feat_dim - 2 主成分作为节点特征向量
        pca = PCA(n_components=all_dim - 2)
        Relative_variation = pca.fit_transform(Relative_variation[:, :all_dim - 2])
        # Relative_variation = Relative_variation[:, all_dim - 2]
        # 1: average to one dimension
        allx_ = StandardScaler().fit_transform(allx)
        trainAttributes = np.average(allx_, axis=1).reshape((len(allx), 1))
        trainAttributes = np.concatenate([trainAttributes, Relative_variation], axis=1)
...
def genenet_attribute_singleCell_all_feat_test(allx, all_dim="758"):
    if allx.shape[1] < cmd_args.feat_dim - 2:
        raise Exception("The dim of Gene Expression is less than feat_dim, please resetup the feat_dim!!!!!")
    if all_dim > 2:
        Relative_variation = np.zeros((allx.shape[0], allx.shape[1] - 1))
        for i in range(allx.shape[1] - 1):
        ...
        #   提取cmd_args.feat_dim - 2 主成分作为节点特征向量
        pca = PCA(n_components=all_dim - 2)
        Relative_variation = pca.fit_transform(Relative_variation)
        # Relative_variation = Relative_variation[:, :all_dim - 2]
        # 1: average to one dimension
        allx_ = StandardScaler().fit_transform(allx)
        trainAttributes = np.average(allx_, axis=1).reshape((len(allx), 1))
...
def subgraph_extraction_labeling_RF_SVM(ind, A, h: int = 1, max_nodes_per_hop=None, node_information=None, max_node_of_graph=8):
    # extract the h-hop enclosing subgraph around link 'ind'
    dist = 0
    nodes = set([ind[0], ind[1]])
    visited = set([ind[0], ind[1]])
    ...
    if node_information is not None:
        features = node_information[nodes]
    features = np.concatenate([features, labels.reshape(features.shape[0], 1)], axis=1)
    temp_features = np.zeros((max_node_of_graph, features.shape[1]))
    if len(nodes) <= max_node_of_graph:
        temp_features[:len(nodes), :] = features
        features = temp_features
    else:
        temp_features = features[:max_node_of_graph, :]
        features = temp_features
...
def TP_FP_TN_FN(cnf_matrix_agent0, total_num, Agent_name,return_mcc=False):
    # result = []
...
def network_info(network, TF_num=None):
    ...
    # print(ij)
    print("****************************************************")
def sample_neg_binary(net, test_ratio=0.1, TF_num=333, train_pos=None, test_pos=None, max_train_num=None):
...
def sample_all_gene_paris(gold_net, noisy_net, TF_num=334):
    label0 = ([], [])
    label1 = ([], [])
    label2 = ([], [])
    label3 = ([], [])
    ...
    for i in tqdm(range(len(noisy_row))):
        tempcount = count
        for j in range(len(gold_row)):
            if gold_row[j] == noisy_row[i] and gold_col[j] == noisy_col[i]:
                count += 1
                if net[gold_row[j], gold_col[j]] == 1 or net[gold_col[j], gold_row[j]] == 1:
                    if net[gold_row[j], gold_col[j]] == net[gold_col[j], gold_row[j]]:
                        label2[0].append(noisy_row[i])
                        label2[1].append(noisy_col[i])
                    else:
                        if net[gold_col[j], gold_row[j]] == 1:
                            label1[0].append(noisy_row[i])
                            label1[1].append(noisy_col[i])
                        else:
                            label3[0].append(noisy_row[i])
                            label3[1].append(noisy_col[i])
        if count - tempcount == 0:
            label0[0].append(noisy_row[i])
            label0[1].append(noisy_col[i])
    # 将label0、label1、label2、label3划分为训练集
    train_label0 = (label0[0][:], label0[1][:])
    train_lable1 = (label1[0][:], label1[1][:])
    train_lable2 = (label2[0][:], label2[1][:])
    ...
    return train_lable3, train_label0, train_lable1, train_lable2

def Cross_3_V_sample_neg_binary_allPairs(net, max_train_num=None, Partion=3, TF_num=410):
    undirected_daj = net.todense()
    row, col, _ = ssp.find(net)
    for i in range(len(row)):
        undirected_daj[row[i], col[i]] = 1
    ...
    if Partion == 0:
        test_neg = (label0[0][:size_test_neg_end], label0[1][:size_test_neg_end])
        test_pos = (label1[0][:size_test_pos_end], label1[1][:size_test_pos_end])
        ...
        test_neg = (label0[0][size_test_neg_start:size_test_neg_end], label0[1][size_test_neg_start:size_test_neg_end])
        test_pos = (label1[0][size_test_pos_start:size_test_pos_end], label1[1][size_test_pos_start:size_test_pos_end])
        ...
        test_neg = (label0[0][size_test_neg_start:], label0[1][size_test_neg_start:])
        test_pos = (label1[0][size_test_pos_start:], label1[1][size_test_pos_start:])
    ...
    # 将label0、label1、label2、label3划分为训练集
    train_neg = (label0[0][:], label0[1][:])
    train_pos = (label1[0][:], label1[1][:])


    return train_pos, train_neg, test_pos, test_neg


def Cross_3_V_sample_neg_binary(net, max_train_num=None, Partion=3):
    undirected_daj = net.todense()
    row, col, _ = ssp.find(net)
    for i in range(len(row)):
        undirected_daj[row[i], col[i]] = 1
...
def extractLinks2subgraphs_binary(Atrain, train_pos,train_neg,
                                   h: int = 1, max_nodes_per_hop=None,
                                   train_node_information=None):
    # extract enclosing subgraphs
    max_n_label = {'value': 0}
    ...
    print('Extract enclosed subgraph...')
    train_graphs = helper(Atrain, train_neg, 0, train_node_information) + \
                   helper(Atrain, train_pos, 1, train_node_information)
...
def extractLinks2subgraphs_balance_SVM(Atrain, train_pos,train_neg,
    ...
    def helper(A, links, g_label, node_information):
        ...
        label_list = []
        for i, j in tqdm(zip(links[0], links[1])):
            _, n_features = subgraph_extraction_labeling_RF_SVM((i, j), A, h, max_nodes_per_hop, node_information)
            g_list.append(n_features.flatten())
...
def extractLinks2subgraphs_binary_SVM(Atrain, train_pos,train_neg,
                                   h: int = 1, max_nodes_per_hop=None,
                                   train_node_information=None):
    # extract enclosing subgraphs
    def helper(A, links, g_label, node_information):
    ...
    print('Extract enclosed subgraph...')
    train_graphs0, train_labels0 = helper(Atrain, train_neg, 0, train_node_information)
    train_graphs3, train_labels3 = helper(Atrain, train_pos, 1, train_node_information)
    train_graphs = train_graphs0 + train_graphs3
    train_labels = train_labels0 + train_labels3
...
def SVM_RF_genenet_attribute_Dream(allx):
    if allx.shape[1] < cmd_args.feat_dim:
        raise Exception("The dim of Gene Expression is less than feat_dim, please resetup the feat_dim!!!!!")
    if cmd_args.feat_dim > 2:
        Relative_variation = np.zeros((allx.shape[0], cmd_args.feat_dim - 2))
        ...
        return np.average(StandardScaler().fit_transform(allx), axis=1).reshape((len(allx), 1))
def SVM_RF_genenet_attribute_SingleCell(allx):
    if allx.shape[1] < cmd_args.feat_dim:
        raise Exception("The dim of Gene Expression is less than feat_dim, please resetup the feat_dim!!!!!")
    if cmd_args.feat_dim > 2:
        # Relative_variation = np.zeros((allx.shape[0], allx.shape[1] - 1))
...
def writeReslut2File_dream(acc, precision, recall, F1, roc, pr, dataset_type='dream', network_type="MI", test_dataset="net3", threshold=0.3, hop=1, location="last", cross_val=True, experiment_count=0):
    if dataset_type == "DREAM":
        ...
        file.write(str(pr[1]) + "\n")
        file.close()
def writeReslut2FileBinary(acc, precision, recall, MCC, roc, pr, dataset_type='DREAM', train_dataset="net4",
    ...
    file = open(file_path + filename, "a")
    if experiment_count == 0:
        str0 = "Times\tAccuary\tPrecision\tRecall\tMCC\tAUROC\tAUPR"
        file.write(str0 + "\n")
    file.write(str(experiment_count) + "\t" + str(acc) + "\t")
    str1 = str(precision) + "\t" + str(recall) + "\t" + str(MCC) + "\t" + str(roc) + "\t" + str(pr) + "\n"
    file.write(str1)
    file.close()
