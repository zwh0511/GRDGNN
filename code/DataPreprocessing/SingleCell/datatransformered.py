...
def process_data(gold_type, dataset_name, num_gene):
    ...
    data_root_path = os.path.join(root_path, "data/SingleCell/raw/{}/{}/{}/".format(goldNetwork_type[gold_type], networkDataset_name[dataset_name], nums_gene[num_gene]))
    data_root_savePath = os.path.join(root_path, "data/SingleCell/processed/{}/{}/{}/".format(goldNetwork_type[gold_type], networkDataset_name[dataset_name], nums_gene[num_gene]))

    os.makedirs(os.path.dirname(data_root_savePath), exist_ok=True)
    ...
    # 保存 target_newId.tsv
    target_newId_path = os.path.join(data_root_savePath, "target_newId.tsv")
    with open(target_newId_path, "w") as targets_f:
        targets_f.write("#ID\tName\n")
        for i, index in enumerate(newIndex):
            targets_f.write(f"G{i}\t{index}\n")

    # 保存 TF_newId.tsv
    TF_newId_path = os.path.join(data_root_savePath, "TF_newId.tsv")
    with open(TF_newId_path, "w") as tfs_f:
        for i, index in enumerate(newIndex[:nums_TF]):
            tfs_f.write(f"G{i}\t{index}\n")

    GeneDict = {index: f"G{i}" for i, index in enumerate(newIndex)}
    GeneDict_1 = {index: i for i, index in enumerate(newIndex)}
    GeneDict_2 = {i: f"G{i}" for i in range(len(newIndex))}

    # 保存 goldStandardNet.tsv
    goldStandardNet_path = os.path.join(data_root_savePath, "goldStandardNet.tsv")
    with open(goldStandardNet_path, "w") as goldNet_f:
        for row in netwoData.values:
            str3 = f"{GeneDict[row[0]]}\t{GeneDict[row[1]]}\t1"
    ...
    # 保存 expression.tsv
    expression_path = os.path.join(data_root_savePath, "expression.tsv")
