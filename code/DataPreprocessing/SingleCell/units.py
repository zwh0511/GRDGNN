...
def BuildNetworksBySettingNumberOfEdges(wik, nodes, average=3):
    flattened = wik.flatten()  # 将矩阵展平为一维数组
    sorted_indices = np.argsort(flattened)[::-1]  # 根据值从大到小对数组进行排序，返回排序后的索引
    sorted_indices[nodes * average:] = 0  # 将前num个值之外的索引设置为0
    flattened.fill(0)  # 将展平后的数组全部设置为0
    flattened[sorted_indices] = 1  # 将前num个值对应的索引设置为1
    reshaped = flattened.reshape(wik.shape)  # 将一维数组重新转换为与原矩阵相同的形状
    return ssp.csc_matrix(reshaped)

def BuildNetworksBySettingNumberOfEdges_undirected(wik, num_goldEdges, average=3):
    flattened = wik.flatten()  # 将矩阵展平为一维数组
    sorted_indices = np.argsort(flattened)[::-1]  # 根据值从大到小对数组进行排序，返回排序后的索引
    sorted_indices[num_goldEdges * average:] = 0  # 将前num个值之外的索引设置为0
    flattened.fill(0)  # 将展平后的数组全部设置为0
    flattened[sorted_indices] = 1  # 将前num个值对应的索引设置为1
    undirected_daj = flattened.reshape(wik.shape)  # 将一维数组重新转换为与原矩阵相同的形
    row, col, _ = ssp.find(undirected_daj)
    for i in range(len(row)):
        undirected_daj[row[i], col[i]] = 1
        undirected_daj[col[i], row[i]] = 1
    return ssp.csc_matrix(undirected_daj)
...
def load_eprData(filename, rownum, colnum, outdim=256):
    ...
    expr_data = pd.read_csv(filename, delimiter="\t", header=None, skiprows=1).values
    data = expr_data[:, 1:]
    data = scale(data)
