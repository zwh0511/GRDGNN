            ...
            if i == 0 and j != 3:
                ...
                for k in range(0, 2):
                    ...
                    if os.path.exists(wik_path):
                        ...
                        _, wik = SSE_Caculate_Order3_Pearson(data, data_path, num_TF=TFDict[networkDataset_name[j]])
                    for ii in range(19, 20):
                        networks = uf.BuildNetworksBySettingNumberOfEdges(wik=wik, nodes=goldGRN.shape[0], average=ii)
                        np.save(data_path + 'Pearson_avgDegree_' + str(ii) + '_Networks_Order3_TF.npy', networks)
                        networks = networks.todense()
                        np.savetxt(data_path + "Pearson_avgDegree_" + str(ii) + "_Networks_Order3_TF.csv", networks, delimiter=",")
                        print("==" * 20)
