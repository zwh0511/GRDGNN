    ...
    for i in tqdm.tqdm(range(num_TF)):
        for j in range(data.shape[1]):
            if i != j:
                ...
                Y = torch.tensor([torch.sum(ys), torch.sum(xs * ys), torch.sum(xs ** 2 * ys), torch.sum(xs ** 3 * ys)], device=device)
        ...
        if i % np.floor(num_TF / 100) == 0:
            print("The completion rate is: " + str(i / num_TF * 100) + "%")
