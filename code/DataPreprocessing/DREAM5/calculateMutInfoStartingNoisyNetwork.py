...
def main(args):
    ...
    if os.path.exists(wik_path):
        ...
        wik = uf.Normalization_MutIfo(SSE, data_path)
    else:
        _, wik = SSE_Caculate_Order3_MutIfo(data, data_path, num_TF=TFDict[args.dataset])
    temp = 5
    for i in range(20):
        temp = temp + 5
        Threshould = (temp / 100)
        if Threshould == 0.3 or Threshould == 0.8:
            ...
            np.save(data_path + 'MutInfo_Threshold_' + str(Threshould) + '_Networks_Order3_TF.npy', networks)
            networks = networks.todense()
            np.savetxt(data_path + "MutInfo_Threshold_" + str(Threshould) + "_Networks_Order3_TF.csv", networks, delimiter=",")
