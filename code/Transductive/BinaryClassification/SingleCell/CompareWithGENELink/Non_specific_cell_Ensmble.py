...
from code.Transductive.BinaryClassification.SingleCell.CompareWithGENELink import units as uf
from code.Transductive.BinaryClassification.SingleCell.CompareWithGENELink.graph_sampler import GRDGrahpUseGeneLinkDataset, DGraphSampler
from torch.utils.data import DataLoader
from code.Transductive.BinaryClassification.SingleCell.CompareWithGENELink.DGCNmodule import SoftPoolingGcnEncoder
...
def LoadData(args,dataset_type="singleCell",
    ...
    Dream_Pear_train_P = GRDGrahpUseGeneLinkDataset(root=root_path, dataset_type=dataset_type, network_type="Pearson", scGRN_type=scGRN_type,
                                  cross_val_location=location,
                                  dataset_name=dataset_name, threshold=threshold, hop=hop, max_nodes_per_hop=10,
                                  use_embedding=False, embedding_dim=1,
                                  all_TF_gene_pairs=False, test_flag=False, cross_val=cross_val, gene_num=gene_num,
                                  avgDegree=avgDegree, TFs_num=TFDict[dataset_name])

    Dream_Pear_test_P = GRDGrahpUseGeneLinkDataset(root=root_path, dataset_type=dataset_type, network_type="Pearson", scGRN_type=scGRN_type,
                                 cross_val_location=location,
                                 dataset_name=dataset_name, threshold=threshold, hop=hop, max_nodes_per_hop=10,
                                 all_TF_gene_pairs=False,
                                 use_embedding=False, embedding_dim=1,
                                 test_flag=True, cross_val=cross_val, gene_num=gene_num, avgDegree=avgDegree,
                                 TFs_num=TFDict[dataset_name])

    Dream_Pear_train_MI = GRDGrahpUseGeneLinkDataset(root=root_path, dataset_type=dataset_type, network_type="MI", scGRN_type=scGRN_type,
                                   cross_val_location=location,
                                   dataset_name=dataset_name, threshold=threshold, hop=hop, max_nodes_per_hop=10,
                                   use_embedding=False, embedding_dim=1,
                                   all_TF_gene_pairs=False, test_flag=False, cross_val=cross_val, gene_num=gene_num,
    ...
    Dream_Pear_test_MI = GRDGrahpUseGeneLinkDataset(root=root_path, dataset_type=dataset_type, network_type="MI", scGRN_type=scGRN_type,
                                  cross_val_location=location,
                                  dataset_name=dataset_name, threshold=threshold, hop=hop, max_nodes_per_hop=10,
                                  all_TF_gene_pairs=False,
                                  use_embedding=False, embedding_dim=1,
...
def main(dataset_type="singleCell",
    ...
    #                             network_type="GeneLinkOriginalDatase_GRDGNN_noisyNet_Newpool",
    ...
    print("The AUROC: {}, the AUPRC: {}".format(str(roc_auc), str(aupr)))

if __name__ == "__main__":
    for gold_type in range(0, 2):
