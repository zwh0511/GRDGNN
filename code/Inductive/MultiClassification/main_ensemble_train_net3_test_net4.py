...
from code.Inductive.MultiClassification.DGCNmodule import SoftPoolingGcnEncoder
...
def LoadData(args):
    ...
    Dream_Pear_train_P = GRDGrahp(root=root_path, dataset_type=dataset_type, network_type="Pearson", scGRN_type=scGRN_type, cross_val_location=location,
                                dataset_name=train_dataset, threshold=threshold, hop=hop, max_nodes_per_hop=10,
                                use_embedding=use_embedding, embedding_dim=1,  balance=train_balance,
                                all_TF_gene_pairs=False, test_flag=False, cross_val=cross_val, gene_num=gene_num, avgDegree=avgDegree, TFs_num=TFDict[train_dataset])

    Dream_Pear_test_P = GRDGrahp(root=root_path, dataset_type=dataset_type, network_type="Pearson", scGRN_type=scGRN_type, cross_val_location=location,
                               dataset_name=test_dataset, threshold=threshold, hop=hop, max_nodes_per_hop=10, all_TF_gene_pairs=False,
                               use_embedding=use_embedding, embedding_dim=1,
                               test_flag=True, cross_val=cross_val, gene_num=gene_num, avgDegree=avgDegree, TFs_num=TFDict[test_dataset])

    Dream_Pear_train_MI = GRDGrahp(root=root_path, dataset_type=dataset_type, network_type="MI", scGRN_type=scGRN_type, cross_val_location=location,
                                dataset_name=train_dataset, threshold=threshold, hop=hop, max_nodes_per_hop=10,
                                use_embedding=use_embedding, embedding_dim=1, balance=train_balance,
                                all_TF_gene_pairs=False, test_flag=False, cross_val=cross_val, gene_num=gene_num, avgDegree=avgDegree, TFs_num=TFDict[train_dataset])

    Dream_Pear_test_MI = GRDGrahp(root=root_path, dataset_type=dataset_type, network_type="MI", scGRN_type=scGRN_type, cross_val_location=location,
                               dataset_name=test_dataset, threshold=threshold, hop=hop, max_nodes_per_hop=10, all_TF_gene_pairs=False,
                               use_embedding=use_embedding, embedding_dim=1,
                               test_flag=True, cross_val=cross_val, gene_num=gene_num, avgDegree=avgDegree, TFs_num=TFDict[test_dataset])
...
def main(iter=0):
    ...
    #                     network_type="MulitClassEnsem", train_dataset=train_dataset,
