...
parser.add_argument('--testdata-name', default='net4', help='test network name')
...
def main(iter):
    if args.traindata_name is not None:
        ...
        if dataset_type == "DREAM":
            ...
            # Prepare Training
            trainNet_ori = np.load(os.path.join(root_path, '{}/goldGRN_{}.csc'.format(args.traindata_name, args.traindata_name)),
                                   allow_pickle=True)
            trainGroup = np.load(os.path.join(root_path, '{}/goldExpression_{}.allx'.format(args.traindata_name, args.traindata_name)),
                                 allow_pickle=True)
            trainNet = np.load(
                root_path + "{}/Pearson_Threshold_{}_Networks_Order3_TF.npy".format(args.traindata_name, str(threshold)),
            ...
            # Prepare Testing
            testNet_ori = np.load(os.path.join(root_path, '{}/goldGRN_{}.csc'.format(args.testdata_name, args.testdata_name)),
                                  allow_pickle=True)
            testGroup = np.load(os.path.join(root_path, '{}/goldExpression_{}.allx'.format(args.testdata_name, args.testdata_name)),
                                allow_pickle=True)
            testNet = np.load(
                root_path + "{}/Pearson_Threshold_{}_Networks_Order3_TF.npy".format(args.testdata_name, str(threshold)),
                allow_pickle=True).tolist()
    ...
    #                     network_type="MultiClassSVM", train_dataset=train_dataset,
    ...
    #                     [micro_auroc, macro_auroc],
    #                     [micro_aupr, macro_aupr], dataset_type=dataset_type,
    #                     network_type="MultiClassRF", train_dataset=train_dataset,
    #                     scGRN_type=scGRN_type, gene_num=gene_num,
    #                     test_dataset=test_dataset, threshold=threshold,
    #                     hop=hop, location=location, cross_val=cross_val,
    #                     experiment_count=iter)
