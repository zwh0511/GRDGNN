...
def LoadData(args):
    ...
    for ii in range(2):
        test_dataset = dreamDataset_name[ii]
        test_P = GRDGrahp(root=root_path,dataset_type="DREAM", network_type="Pearson", scGRN_type="STRING",
                                     cross_val_location=location,undirected=undirected,
        ...
        test_MI = GRDGrahp(root=root_path,dataset_type="DREAM", network_type="MI", scGRN_type="STRING",
                                      cross_val_location=location,undirected=undirected,
                                      dataset_name=test_dataset, threshold=threshold, hop=hop, max_nodes_per_hop=10,
                                      all_TF_gene_pairs=False, test_flag=True,
                                      balance=test_balance, cross_val=cross_val, gene_num=gene_num, avgDegree=avgDegree,
...
def main(iter):
    ...
    uf.writeReslut2File(acc, Precision_score, Recall_score, F1_scor,
                        ...
                        network_type="Ensem_undirected", train_dataset="SingleCell",
                        scGRN_type=scGRN_type, gene_num=gene_num,
                        test_dataset="DREAM", threshold=threshold,
