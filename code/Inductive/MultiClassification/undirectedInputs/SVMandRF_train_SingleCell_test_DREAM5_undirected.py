...
def loadDreamTraindataset(root_path, traindata_name):
    ...
    trainGroup = np.load(os.path.join(root_path, 'goldGRN/goldExpression_{}.allx'.format(traindata_name)),
                         allow_pickle=True)
    trainNet = np.load(
        root_path + "{}/Pearson_Threshold_{}_Networks_Order3_TF_undirected.npy".format(traindata_name, str(threshold)),
...
def main(iter):
    ...
    root_path1 = "/root/sdc/guozy/GRDGNN/data/SingleCell/processed/"
    root_path2 = "/root/sdc/guozy/GRDGNN/data/DREAM/processed/"
    for i in range(7):
        temp_train_graphs, temp_train_labels = loadSinglCellTraindata(root_path1, networkDataset_name[i])
        if i == 0:
    ...
    for j in range(2):
        temp_test_graphs, temp_test_labels = loadDreamTestdataset(root_path2, dreamDataset_name[j])
        if j == 0:
    ...
    uf.writeReslut2File(acc, Precision_score, Recall_score, F1_scor,
                        ...
                        network_type="SVM_undirected", train_dataset="SingleCell",
    ...
    uf.writeReslut2File(acc, Precision_score, Recall_score, F1_scor,
                        [micro_auroc, macro_auroc],
                        [micro_aupr, macro_aupr], dataset_type=dataset_type,
                        network_type="RF_undirected", train_dataset="SingleCell",
                        scGRN_type=scGRN_type, gene_num=gene_num,
                        test_dataset="DREAM", threshold=threshold,
                        hop=hop, location=location, cross_val=cross_val,
                        experiment_count=iter)
