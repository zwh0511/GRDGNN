...
def LoadData(args):
    # loading the training data: Mouse dataset
    root_path = uf.get_project_dir("GRDGNN") + "data"
    for i in range(2, 7):
        train_dataset = networkDataset_name[i]
        train_P= GRDGrahp(root=root_path, dataset_type=dataset_type, network_type="Pearson", scGRN_type=scGRN_type,
        ...
        train_MI= GRDGrahp(root=root_path, dataset_type=dataset_type, network_type="MI", scGRN_type=scGRN_type,
                   cross_val_location=location,undirected=undirected,
                   dataset_name=train_dataset, threshold=threshold, hop=hop, max_nodes_per_hop=10,
                   all_TF_gene_pairs=False, test_flag=False,
                   balance=train_balance, cross_val=cross_val, gene_num=gene_num, avgDegree=avgDegree,
                   TFs_num=TFDict[train_dataset])
        if i == 2:
            Dream_Pear_train_P = train_P
            Dream_Pear_train_MI = train_MI
        else:
            Dream_Pear_train_P = Dream_Pear_train_MI + train_P
            Dream_Pear_train_MI = Dream_Pear_train_MI + train_MI

    for ii in range(0, 2):
        test_dataset = networkDataset_name[ii]
        test_P = GRDGrahp(root=root_path, dataset_type="SingleCell", network_type="Pearson", scGRN_type=scGRN_type,
                                     cross_val_location=location,undirected=undirected,
        ...
        test_MI = GRDGrahp(root=root_path, dataset_type="SingleCell", network_type="MI", scGRN_type=scGRN_type,
                                      cross_val_location=location,undirected=undirected,
                                      dataset_name=test_dataset, threshold=threshold, hop=hop, max_nodes_per_hop=10,
                                      all_TF_gene_pairs=False, test_flag=True,
                                      balance=test_balance, cross_val=cross_val, gene_num=gene_num, avgDegree=avgDegree,
                                      TFs_num=TFDict[test_dataset])
        if ii == 0:
            Dream_Pear_test_P = test_P
            Dream_Pear_test_MI = test_MI
        else:
            Dream_Pear_test_P = Dream_Pear_test_P + test_P
            Dream_Pear_test_MI = Dream_Pear_test_MI + test_MI
...
def main(iter):
    ...
    uf.writeReslut2File(acc, Precision_score, Recall_score, F1_scor,
                        ...
                        network_type="Ensem", train_dataset="Human",
                        scGRN_type=scGRN_type, gene_num=gene_num,
                        test_dataset="Mouse", threshold=threshold,
